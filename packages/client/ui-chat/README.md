...
Render a browser chat from recorded Session conversations, including historical images, localized actions, and restored scroll position. Compact display folds finished process rows during a Turn and completed-turn rows after `turn/end`, keeping the trailing answer and independently useful context visible; packed historical Assistant runs stay collapsed. Local transcript and steering submissions appear immediately, keep their original surface, and disappear atomically when authoritative Session records arrive; queued submissions stay outside Chat. The package does not assemble or modify model requests.
