...
在浏览器中渲染已记录的 Session 对话，包括历史图片、本地化操作和滚动位置恢复。紧凑显示会在轮次进行中收起已完成的过程行，并在 `turn/end` 后收起已完成轮次的过程行，同时保持当前正文和独立有用的上下文可见；已打包的历史 Assistant 连续消息保持收起。本地 transcript（文本记录）与 steering（中途引导）提交会立即显示并保留在原区域，在权威会话记录到达时原子地消失；排队中的提交始终不进入 Chat。本包不组装或修改模型请求。
