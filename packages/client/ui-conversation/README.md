...
The plan strip renders the Session `todos` projection in the same dock and owns no plan data: an empty or retired list removes it, and one row shows each item's status. Its expansion preference is one browser-wide persisted value (`dsh.conversation.todo-expanded`), so the strip keeps the posture the user left it in across Session switches and reloads.
